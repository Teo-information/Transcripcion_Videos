...
def main_app():
    ...
    if archivo is not None:
        ...
        # Verificación extra: asegurar que el archivo existe
        if not os.path.exists(destino):
            st.error(f"El archivo {destino} no existe después de guardar. Por favor, intenta de nuevo.")
            st.stop()
        else:
            st.info(f"Archivo listo para procesar: {destino}")
            st.write(f"Contenido de archivos_subidos: {os.listdir(save_folder)}")
    st.divider()
    ...
    if archivo is not None:
        if os.path.exists(destino):
            if st.button("Generar Transcripcion", icon= "📝"):
                ...
                if not os.path.exists(destino):
                    st.error(f"El archivo {destino} no existe en el momento de procesar. Por favor, sube el archivo nuevamente.")
                    st.stop()
                st.write(f"Procesando archivo: {destino}")
                st.write(f"¿Existe el archivo?: {os.path.exists(destino)}")
                st.write(f"Contenido de archivos_subidos: {os.listdir('archivos_subidos')}")
